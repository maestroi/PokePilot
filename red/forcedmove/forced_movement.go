...
// Point is a Red game-tile coordinate. It intentionally does not depend on
// the portable world package: this adapter data is consumed by navigation, but
// must not make Red's ROM packages depend back on the generic graph layer.
type Point struct {
	X int
	Y int
}
...
const (
	RocketHideoutB2F uint8 = 0xC8
	RocketHideoutB3F uint8 = 0xC9
	ViridianGym      uint8 = 0x2D
)

// tables are Red adapter facts decoded from the map scripts. The portable
// navigation layer only consumes trigger -> landing transitions; it does not
// know what a spinner/arrow tile is or which game supplied them.
var tables = map[uint8]map[[2]int]Point{
	RocketHideoutB2F: {
...
// Landing returns the deterministic landing selected by Red's script after
// the player enters trigger (x,y). false means the tile is ordinary movement.
func Landing(mapID uint8, x, y int) (Point, bool) {
	table := tables[mapID]
	if table == nil {
		return Point{}, false
	}
...
// Transitions returns a copy for adapter-level verification.
func Transitions(mapID uint8) map[[2]int]Point {
	src := tables[mapID]
	if len(src) == 0 {
		return nil
	}
	out := make(map[[2]int]Point, len(src))
	for k, v := range src {
		out[k] = v
	}
	return out
