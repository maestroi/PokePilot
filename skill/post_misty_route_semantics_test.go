...
func TestPostMistyRouteTransitionsExposeBillShipAndCutGates(t *testing.T) {
	...
	// ordinary: PivotOnly on that border invented unreachable east-edge
	// crossings from Cerulean's west bank (run-1vgggovqm500x1sw9gnrhtu1l1).
	...
	if _, ok := redRouteTransitionForEdge(enter); ok {
		t.Fatal("Cerulean -> Route 9 must remain ordinary geometry")
	}
}
