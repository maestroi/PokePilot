...
func ViridianGymReady(mem *state.Mem, a wramAddresses) bool {
	inv := a.DecodeInventory(mem)
	return a.DecodeStoryFacts(mem, inv).ViridianGymOpen
}
...
func EnterViridianGym(m *emu.Emu, romData []byte, policy MovePolicy) error {
	if m.Peek8(ram(m).CurMap) == viridianGymMap {
		return nil
	}
	var mem state.Mem
	state.Snapshot(m, &mem)
	if !ViridianGymReady(&mem, ram(m)) {
	...
	if m.Peek8(ram(m).CurMap) != viridianGymMap {
		return fmt.Errorf("skill: EnterViridianGym: stopped on map %#04x", m.Peek8(ram(m).CurMap))
	}
	return nil
}
...
func walkViridianGymToLeader(m *emu.Emu, romData []byte) error {
	if m.Peek8(ram(m).CurMap) != viridianGymMap {
		return fmt.Errorf("skill: ViridianGym: spinner route on map %#04x, want %#04x", m.Peek8(ram(m).CurMap), viridianGymMap)
	}
	h, err := graphForROM(romData).ParseMap(romData, viridianGymMap)
	if err != nil {
		return err
	}
...
func ViridianProgression(m *emu.Emu, romData []byte, policy MovePolicy) error {
	...
	var mem state.Mem
	state.Snapshot(m, &mem)
	progress := ram(m).DecodeProgress(&mem)
	...
	if m.Peek8(ram(m).CurMap) != viridianGymMap {
		...
		mem = advanceUntil(m, ram(m), viridianGymOpenBudget, func(mm *state.Mem, a wramAddresses) bool {
			return ViridianGymReady(mm, ram(m)) && ram(m).Controllable(mm)
		})
		if !ViridianGymReady(&mem, ram(m)) {
	...
	} else if !ViridianGymReady(&mem, ram(m)) {
