...
func TestVictoryRoadProgressionRealROM(t *testing.T) {
	...
	if got := redWram().DecodeProgress(&before).BadgeCount; got != 8 {
	...
	facts := redWram().DecodeStoryFacts(&after, redWram().DecodeInventory(&after))
	...
	if !allPartyCenterRecovered(&after, redWram()) {
