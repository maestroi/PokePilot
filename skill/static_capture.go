...
func staticBall(mem *state.Mem, species uint8) (uint8, bool) {
	// Preserve the unique Master Ball for Mewtwo when ordinary balls exist.
	// If no ordinary ball remains it is still better to use it than strand a
	// one-time target permanently.
...
func CaptureStatic(m *emu.Emu, romData []byte, species uint8, policy MovePolicy) (CatchResult, error) {
	...
	for attempt := 0; attempt < staticRetryCount; attempt++ {
		...
		if err != nil {
			// A throw/menu failure is safe after rollback; try another deterministic
			// frame phase before deciding the source is unusable for this run.
			continue
		}
	}
