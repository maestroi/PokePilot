...
| `pokepilot_get_run_artifact_content` | Fetch one small **inline** artifact's bytes as base64 (a `.state`/`.ram`/JSON checkpoint) for local reproduction; refuses artifacts stored remotely (e.g. `run.gbrun`) or larger than the MCP response cap |
...
`admin.rompilot.app` sits behind Cloudflare Access for its plain `/v1/*` browser routes, including `GET /v1/runs/{id}/artifacts/{name}/content`; an automated agent holding only `POKEPILOT_MCP_TOKEN` gets a 302 to the Access login page there, not a 401. `pokepilot_get_run_artifact_content` exists because `/mcp` calls `pokewall` server-to-server (`c.wallBase`, the private overlay address) the same way every other MCP tool does, so it never crosses the public Access-gated edge. Use it instead of trying to `curl` the content route directly from outside the Access session; it only ever returns the same small inline bytes that route would, bounded by `mcpMaxResponseBytes` (2 MiB), and it still refuses artifacts pokewall itself marks as remotely stored.
