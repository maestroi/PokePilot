...
not vendored here — it is a full checkout at `~/.cache/pokered`, reachable from
the repo root as `pokered/` (a gitignored symlink; `make pokered` recreates it).
...
a path in *this* repository. It is `pokered/scripts/*.asm`.
...
several attempts here have died. Scope the search:
    ...
    grep -rn EVENT_GOT_POKEDEX pokered/scripts        # who checks or sets it
