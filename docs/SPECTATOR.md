...
The page polls the sanitized snapshot every two seconds. A selected live Pokémon Red run also polls validated semantic `RenderState` independently. **Modern** now renders supported overworld, dialogue, generic menu, and battle scenes without changing the run. Overworld movement uses a browser-owned bounded animation clock: authoritative player/NPC positions are interpolated at display refresh, the camera follows fractional tile coordinates, and long reconnects, map changes, rewinds, and large teleports snap directly to current truth instead of replaying stale movement. This timing is independent of emulator speed, so accelerated runs remain watchable without slowing the agent. Battle presentation uses authoritative species, level, HP, status, and active-move/PP state decoded by the Red adapter. The viewer can switch between **Classic** (the default framebuffer view) and **Modern**; the choice is remembered per browser; while Modern is active, a viewer-local theme selector chooses among installed packs such as **RomPilot Modern** and **Retro 16-bit**. Theme choice is stored only in browser-local storage and never changes or restarts the run. Unsupported semantic sub-scenes use the classic framebuffer as a compatibility scene while the viewer's selected mode remains Modern, so temporary transitions no longer look like the preference itself changed. The semantic endpoint is read-only and buffered on the emulator stepping goroutine, so spectator HTTP requests never inspect or mutate gameplay RAM. See `docs/RENDER_THEMES.md` for the versioned pack contract.
