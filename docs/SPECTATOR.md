...
The page polls the sanitized snapshot every two seconds. A selected live Pokémon Red run also polls validated semantic `RenderState` independently and renders supported overworld scenes on a camera-following Canvas. The viewer can switch between **Modern** and **Classic**; while Modern is active, a viewer-local theme selector chooses among installed render packs such as **RomPilot Modern** and **Retro 16-bit**. Theme choice is stored only in browser-local storage and never changes or restarts the run. Missing optional theme semantics inherit from the default pack. Battle/menu/unsupported or unavailable semantic states automatically fall back to the classic framebuffer. The semantic endpoint is read-only and buffered on the emulator stepping goroutine, so spectator HTTP requests never inspect or mutate gameplay RAM. See `docs/RENDER_THEMES.md` for the versioned pack contract.
