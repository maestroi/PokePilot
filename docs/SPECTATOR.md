...
The page polls the sanitized snapshot every two seconds. A selected live Pokémon Red run also polls validated semantic `RenderState` independently. The **Classic** emulator framebuffer is the default view; viewers can opt into the semantic **Gold / Silver** presentation for supported overworld, dialogue, menu, and battle scenes without changing the run. Overworld movement uses a browser-owned bounded animation clock: authoritative player/NPC positions are interpolated at display refresh, while reconnects, map changes, rewinds, and large teleports snap directly to current truth. Battle presentation uses authoritative species, level, HP, status, and move/PP state decoded by the Red adapter with Gold-version sprites. The viewer can switch between **Classic** and the semantic **Gold / Silver** view, and that choice is remembered per browser; Tiny Town remains available as the CC0 alternate theme. Theme choice is browser-local and never changes or restarts the run. The semantic endpoint is read-only and buffered on the emulator stepping goroutine, so spectator HTTP requests never inspect or mutate gameplay RAM. See `docs/RENDER_THEMES.md` for the theme contract and provenance.
