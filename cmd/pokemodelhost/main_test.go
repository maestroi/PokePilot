...
func TestLifecycleRejectsSwitchWhileRunActive(t *testing.T) {
	...
	if resp.StatusCode != http.StatusAccepted { t.Fatalf("acquire status = %d", resp.StatusCode) }
	deadline := time.Now().Add(time.Second)
	for time.Now().Before(deadline) {
		if service.status().State == "ready" { break }
		time.Sleep(time.Millisecond)
	}
	if got := service.status(); got.State != "ready" || got.ActiveLeases != 1 { t.Fatalf("status = %#v", got) }

	resp, _ = post("/v1/load", loadRequest{DeploymentID: "qwen-9b"})
	if resp.StatusCode != http.StatusConflict { t.Fatalf("switch status = %d, want 409", resp.StatusCode) }

	resp, _ = post("/v1/leases/release", leaseRequest{RunID: "run-a", DeploymentID: "qwen-4b"})
	if resp.StatusCode != http.StatusOK { t.Fatalf("release status = %d", resp.StatusCode) }
	resp, _ = post("/v1/load", loadRequest{DeploymentID: "qwen-9b"})
	if resp.StatusCode != http.StatusAccepted { t.Fatalf("second load status = %d", resp.StatusCode) }
}
...
func TestLifecycleBearerAuth(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	resp.Body.Close()
	if resp.StatusCode != http.StatusOK { t.Fatalf("authorized status = %d", resp.StatusCode) }
}
