...
func init() { registerSemanticObservationAdapter(redSemanticObservationAdapter{}) }

var redMoveTypeNames = map[uint8]string{
	0x00: "normal", 0x01: "fighting", 0x02: "flying", 0x03: "poison",
	0x04: "ground", 0x05: "rock", 0x06: "flying", 0x07: "bug", 0x08: "ghost",
...
func (redSemanticObservationAdapter) Observe(m *emu.Emu, romData []byte, profile game.GameProfile) (Observation, error) {
	...
	gs := state.Read(m, &mem)
	obs := Observation{
		Map: uint8(base.NativeMapID), Location: base.Location, MapName: base.MapName,
		X: base.X, Y: base.Y, Facing: base.Facing, Controllable: base.Controllable,
		InBattle: base.InBattle, PartyCount: len(base.Party), Money: base.Money,
		RespawnPlace: base.RespawnPlace, Party: make([]PartyMon, len(base.Party)),
		Badges: append([]string(nil), base.Badges...), Events: append([]string(nil), base.Events...),
		Story: append(ProgressState(nil), base.Story...), BlackedOut: base.BlackedOut,
		LeadMoves: []Move{}, LeadPP: []uint8{}, Bag: []Item{}, FieldCapabilities: []FieldCapability{},
		RecentDialogue: []string{}, History: []RoundRecord{}, Failures: []Failure{}, Requirements: []Requirement{},
		PokedexOwned: []SpeciesID{}, PokedexSeen: []SpeciesID{},
	}
	obs.PokedexOwned, obs.PokedexSeen = ProjectPokedex(romData, gs.Pokedex)
	for i, mon := range base.Party {
		obs.Party[i] = PartyMon{Species: SpeciesID(mon.Species), Level: mon.Level, Experience: mon.Experience, HP: mon.HP, MaxHP: mon.MaxHP, Status: mon.Status}
	}
	...
	for _, cap := range skill.FieldCapabilities(&mem) {
		obs.FieldCapabilities = append(obs.FieldCapabilities, FieldCapability{
			Name: CapabilityID(semanticPlace(cap.Name)), Badge: cap.Badge.String(), BadgeOwned: cap.BadgeOwned,
			HMOwned: cap.HMOwned, Learned: cap.Learned, PartySlot: cap.PartySlot, Usable: cap.Usable,
