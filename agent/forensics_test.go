...
func TestEvictRAMCapturesKeepsNewestPairs(t *testing.T) {
	...
	if err := evictRAMCaptures(dir, failurePrefix, 2); err != nil {
		t.Fatal(err)
	}
...
func TestCaptureStallDisabledDoesNotTouchEmulator(t *testing.T) {
	t.Setenv(RAMForensicsDirEnv, "")
	if err := CaptureStall(nil, "reach pewter city", "no progress"); err != nil {
		t.Fatalf("disabled stall capture = %v, want nil", err)
	}
}

// Stall evidence is rarer than failure evidence and must survive a burst of
// failures, so each prefix gets its own ring rather than sharing one budget.
func TestEvictRAMCapturesLeavesOtherPrefixesAlone(t *testing.T) {
	dir := t.TempDir()
	write := func(base string) {
		t.Helper()
		for _, ext := range []string{".ram", ".json"} {
			if err := os.WriteFile(filepath.Join(dir, base+ext), []byte("x"), 0o644); err != nil {
				t.Fatal(err)
			}
		}
	}
	write("failure-frame-0000000001-x")
	write("failure-frame-0000000002-x")
	write("stall-frame-0000000001-reach-pewter-city")

	if err := evictRAMCaptures(dir, failurePrefix, 1); err != nil {
		t.Fatal(err)
	}
	if _, err := os.Stat(filepath.Join(dir, "failure-frame-0000000001-x.ram")); !os.IsNotExist(err) {
		t.Fatalf("oldest failure survived eviction: %v", err)
	}
	for _, name := range []string{
		"failure-frame-0000000002-x.ram",
		"stall-frame-0000000001-reach-pewter-city.ram",
		"stall-frame-0000000001-reach-pewter-city.json",
	} {
		if _, err := os.Stat(filepath.Join(dir, name)); err != nil {
			t.Fatalf("%s was evicted by a failure-ring eviction: %v", name, err)
		}
	}
}
