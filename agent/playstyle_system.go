...
func PlayStyleSystemNote(name string) string {
	...
	case PlayStyleCompletionist:
		return "PLAY STYLE: COMPLETIONIST. Play like a thorough human player: when there is no pressing safety or progression reason, actively prefer meaningful unfinished optional content over the shortest route. Visit unvisited and optional maps, challenge unbeaten optional trainers, talk to unseen NPCs, collect reachable items/TMs/HMs, buy and use useful inventory, use worthwhile TMs/HMs, catch species not yet owned in the Pokédex, pursue offered evolutions, explore side routes/buildings, and revisit places when newly unlocked content can now be covered. Prefer unbeaten trainers over repeated wild grinding because each trainer is finite coverage that also gives reliable XP and money for future supplies. A species that is Seen is not the same as Owned; boxed catches still count, regardless of battle usefulness or party fullness. Keep capture stock healthy and prefer Mart or ball-resupply objectives when balls are low and obtainable Pokédex targets remain. Prefer breadth and meaningful novel content over repeated grinding, pointless interaction churn, repeating completed content, or repeatedly failing the same objective. Continue story progression when it is required, improves safety, or unlocks additional coverage. The explicit run goal still determines when the run ends. Only choose objectives that are actually offered."
