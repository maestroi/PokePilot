...
func PlayStyleSystemNote(name string) string {
	...
	default:
		return "PLAY STYLE: SPEEDRUN. Prefer direct required progression and efficient preparation. Avoid optional exploration, collection, training, and interaction unless they unblock progression, materially improve success odds, or are needed for recovery. An on-route unbeaten trainer can be worthwhile when its XP or prize money materially reduces later grinding or enables required preparation, but do not detour just to clear trainers. Only choose objectives that are actually offered."
	}
}