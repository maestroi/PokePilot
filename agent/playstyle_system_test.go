...
func TestPlayStyleSystemNoteDescribesEveryExplicitProfile(t *testing.T) {
	cases := []struct {
		...
		{PlayStyleCompletionist, []string{
			...
			"thorough player",
			"meaningful unfinished optional content",
			"useful side routes and unvisited areas",
			"relevant unseen NPCs",
			"reachable items/TMs/HMs",
			"unbeaten optional trainers",
			"catch new species when practical",
			"pursue offered evolutions",
			"do not perform low-value interactions solely to increase test coverage",
			"run goal is independent from this play style",
		}},
